...
AllowedIPs = ${CLIENT_WG_IPV4}/32,${CLIENT_WG_IPV6}/128" >>"/etc/wireguard/${SERVER_WG_NIC}.conf"
	...
	while true
		do
		    read -p 'Insert the user email to send wireguard VPN conf (If you left blank i'll send to $SYSADMIN_EMAIL): ' CLIENT_EMAIL
		    echo
...
function resendEmail() {
	...
	while true
		do
		    read -p "Insert the user email to resend wireguard VPN conf (If you left blank i'll send to $SYSADMIN_EMAIL): " CLIENT_EMAIL
		    echo
			if [[ $CLIENT_EMAIL == "" ]]; then
				echo "Generating QR easy access..."
				qrencode -s 6 -l H -o "${HOME_DIR}/${SERVER_WG_NIC}-client-${CLIENT_NAME}-QR.png" < "${HOME_DIR}/${SERVER_WG_NIC}-client-${CLIENT_NAME}.conf"
