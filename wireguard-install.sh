...
function installQuestions() {
	...
	until [[ ${DNS_CHOICE} =~ ^[1-4]$ ]]; do
		read -rp "DNS [1]: " -e -i 1 DNS_CHOICE
	done
	case ${DNS_CHOICE} in
	1)
	...
	4)
		until [[ ${CLIENT_DNS_1} =~ ^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$ ]]; do
			read -rp "Primary DNS: " -e CLIENT_DNS_1
		done
		;;
	esac

	until [[ ${CLIENT_DNS_2} =~ ^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$ ]]; do
		read -rp "Second DNS resolver to use for the users (optional): " -e -i 8.8.4.4 CLIENT_DNS_2
...
AllowedIPs = ${CLIENT_WG_IPV4}/32,${CLIENT_WG_IPV6}/128" >>"/etc/wireguard/${SERVER_WG_NIC}.conf"
	...
	while true
		do
		    read -p 'Do you want to send the wireguard VPN conf to user email? (yes/no): ' RESPONSE
            case "${RESPONSE}" in
                [yY]|[yY][eE][sS]) 
                    while true
                        do
                            read -p "Insert the user email to send wireguard VPN conf (If you left blank i'll send to ): " CLIENT_EMAIL
                            echo
...
function resendEmail() {
	...
	while true
		do
		    read -p "Insert the user email to resend wireguard VPN conf (If you left blank i'll send to ): " CLIENT_EMAIL
		    echo
			if [[ $CLIENT_EMAIL == "" ]]; then
				echo "Generating QR easy access..."
				qrencode -s 6 -l H -o "${HOME_DIR}/${SERVER_WG_NIC}-client-${CLIENT_NAME}-QR.png" < "${HOME_DIR}/${SERVER_WG_NIC}-client-${CLIENT_NAME}.conf"
